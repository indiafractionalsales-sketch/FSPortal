...
export async function POST(request: Request) {
  try {
    ...
    // 1. Find the active/flagged attendance record
    ...
    // 2. Fetch Post to calculate distance on checkout
    ...
    const checkInTime = new Date(data.checkIn.timestamp);
    const checkOutTime = new Date();
    const durationMs = checkOutTime.getTime() - checkInTime.getTime();
    const totalDurationMinutes = Math.max(1, Math.round(durationMs / (1000 * 60)));
    ...
    // 3. Update the attendance record to mark it completed
